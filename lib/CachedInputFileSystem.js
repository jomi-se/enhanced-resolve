...
class Storage {
	constructor(duration) {
		...
		this.data = new Map();
		this.rotatingCache = [];
		this.cacheTimeSlotSize = 0;
		if (duration > 0) {
			this.cacheTimeSlotSize = Math.floor(
				this.duration / this.rotatingCache.length
			);
			this.rotatingCache.push(
				new Set(),
				new Set(),
				new Set(),
				new Set(),
			...
			for (let i = 8000; i < duration; i += 500)
				this.rotatingCache.push(new Set());
		}
		this.count = 0;
		/** @type {NodeJS.Timeout | null} */
		this.interval = null;
		this.nextTickTimeUnix = null;
		this.isCurrentlyPassive = true;
		this.tick = this.tick.bind(this);
	}

	ensureTick() {
		if (!this.interval && this.duration > 0 && !this.nextTickTimeUnix)
			this.interval = setInterval(this.tick, cacheTimeSlotSize);
	}
	...
	finished(name, err, result) {
		...
		if (this.duration > 0) {
			...
			const levelData = this.rotatingCache[0];
			levelData.add(name);
			this.count += 1;
			this.ensureTick();
		}
	...
	tick() {
		// Assumes this.cacheTimeSlotSize ms have passed since last `tick`
		// Assumes this.rotatingCache has entries which is false when duration is falsy/less than 0
		const decay = this.rotatingCache.pop();
		for (let item of decay) {
			this.data.delete(item);
		}
		this.count -= decay.size;
		decay.clear();
		this.rotatingCache.unshift(decay);
		if (this.count === 0) {
			if (this.interval) {
			...
			this.nextTickTimeUnix = null;
			return true;
		} else if (this.nextTickTimeUnix) {
			this.nextTickTimeUnix += this.cacheTimeSlotSize;
			const currentTimeUnix = Date.now();
			if (this.nextTickTimeUnix > currentTimeUnix) {
				if (this.interval) {
					clearInterval(this.interval);
				}
				this.nextTickTimeUnix = null;
				this.interval = setInterval(this.tick, this.cacheTimeSlotSize);
				return true;
			}
		} else if (this.isCurrentlyPassive) {
			// Switch off interval while cache isn't being actively used
			if (this.interval) {
				clearInterval(this.interval);
				this.interval = null;
			}

			this.nextTickTimeUnix = Date.now() + this.cacheTimeSlotSize;
		} else {
			this.isCurrentlyPassive = true;
		}
	...
	checkTicks() {
		if (this.nextTickTimeUnix) {
			while (!this.tick());
		}
		this.isCurrentlyPassive = false;
	}
	...
	purge(what) {
		if (!what) {
			...
			this.nextTickTimeUnix = null;
			this.data.clear();
			this.rotatingCache.forEach(level => {
				level.clear();
			});
